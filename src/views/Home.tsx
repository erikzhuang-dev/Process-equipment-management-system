...
function EquipmentView({ isAdmin }: { isAdmin: boolean }) {
  const { language } = useLanguage();
  const [search, setSearch] = useState(""); const [page, setPage] = useState(1); const [historyId, setHistoryId] = useState<number | null>(null); const [statusId, setStatusId] = useState<number | null>(null); const [expandedEquipmentId, setExpandedEquipmentId] = useState<number | null>(null); const [productDrawerOpen, setProductDrawerOpen] = useState(false); const [selectedBusinessUnitCode, setSelectedBusinessUnitCode] = useState<string | null>(null); const [, navigate] = useLocation(); const inputRef = useRef<HTMLInputElement>(null); const { current: actingIdentity } = useIdentity(); const isAdminIdentity = actingIdentity?.roleKey === "admin"; const approvalPending = trpc.applications.approval.myPending.useQuery(undefined, { enabled: isAdminIdentity, refetchInterval: 30000 }); const approvalPendingCount = approvalPending.data?.length ?? 0; const utils = trpc.useUtils();
