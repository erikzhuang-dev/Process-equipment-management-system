...
function CoreModulesFixture() {
  const { setLanguage } = useLanguage();
  return <><button onClick={() => setLanguage("en")}>English</button><section><p>关键运营说明</p><p>设备详情</p><p>预防性维护</p><p>故障闭环</p><p>备件出入库流水</p><p>用户权限与操作日志</p><p>资产类别</p><p>设备、生产与生命周期参数</p><p>投资与折旧</p><p>治理信息</p><p>OEE（0–1）</p><p>B · 重要</p></section></>;
}
...
describe("LanguageProvider DOM translations", () => {
  ...
  it("translates key dashboard and lifecycle module labels in English mode", async () => {
    ...
    try {
      ...
      expect(container.textContent).toContain("OEE (0–1)");
      expect(container.textContent).toContain("B · Important");
    } finally {
      unmount();
    }
  });
