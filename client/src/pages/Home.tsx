...
type EquipmentForm = {
  ...
  supplier: string; supplierId: string; businessUnitId: string; factoryId: string; hourlyCapacity: string; oee: string; lowOeeReason: string; energyConsumption: string; quantity: string; unitPrice: string; depreciationYears: string; lossFactor: string; investmentIncluded: boolean; notes: string;
};
...
function FormInput({ label, value, onChange, type = "text", required = false }: { label: string; value: string | number; onChange: (value: string) => void; type?: string; required?: boolean }) {
  return <div className="space-y-1.5"><label className="text-sm font-medium text-[#405342]">{label}{required ? " *" : ""}</label><Input type={type} value={value} onChange={event => onChange(event.target.value)} required={required} className="border-[#d9e5d6] bg-white focus-visible:ring-[#6f9a73]" /></div>;
}
...
function InlineEquipmentDetailEditor({ item, businessUnits, factories, suppliers, isAdmin, onSave }: { item: any; businessUnits: any[]; factories: any[]; suppliers: any[]; isAdmin: boolean; onSave: (values: EquipmentForm) => Promise<unknown> }) {
  ...
  const updateDraft = <K extends keyof EquipmentForm>(key: K, value: EquipmentForm[K]) => setValues(current => ({ ...current, [key]: value }));
  ...
  const commitFactory = (value: string) => { const nextValues = { ...values, factoryId: value === "none" ? "" : value }; setValues(nextValues); queueSave(nextValues); };
  const commitSupplier = (value: string) => { const selected = suppliers.find(supplier => String(supplier.id) === value); const nextValues = value === "none" ? { ...values, supplierId: "", supplier: "" } : selected ? { ...values, supplierId: String(selected.id), supplier: selected.name } : values; setValues(nextValues); queueSave(nextValues); };
  const commitInvestment = () => { const nextValues = { ...values, investmentIncluded: !values.investmentIncluded }; setValues(nextValues); queueSave(nextValues); };
  ...
  return <div className="rounded-2xl border border-[#dbe9d7] bg-white p-4 shadow-sm"><div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-semibold text-[#34513a]">{item.code} · 设备详情</p><p className="mt-1 text-xs text-[#829081]">{isAdmin ? "可直接编辑运营与投资字段；切换选择项或离开输入框后系统自动保存。" : "设备运营与投资字段由管理员维护。"}</p></div><span className={`text-xs font-medium ${saveClassName}`}>{saveState === "saving" && <Loader2 className="mr-1 inline h-3.5 w-3.5 animate-spin" />}{saveStatus}</span></div><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{field("所属 BU", <Select value={values.businessUnitId || "none"} onValueChange={commitBusinessUnit} disabled={!isAdmin}><SelectTrigger className={inputClass}><SelectValue /></SelectTrigger><SelectContent><SelectItem value="none">未选择 BU</SelectItem>{businessUnits.map(unit => <SelectItem key={unit.id} value={String(unit.id)}>{unit.code} · {unit.name}</SelectItem>)}</SelectContent></Select>)}{field("工厂", <Select value={values.factoryId || "none"} onValueChange={commitFactory} disabled={!isAdmin}><SelectTrigger className={inputClass}><SelectValue /></SelectTrigger><SelectContent><SelectItem value="none">未选择工厂</SelectItem>{availableFactories.map(factory => <SelectItem key={factory.id} value={String(factory.id)}>{factory.code} · {factory.name}{factory.businessUnitId === null ? "（未归属 BU）" : ""}</SelectItem>)}</SelectContent></Select>)}{field("供应商", <Select value={values.supplierId || (values.supplier ? "legacy" : "none")} onValueChange={commitSupplier} disabled={!isAdmin}><SelectTrigger className={inputClass}><SelectValue /></SelectTrigger><SelectContent><SelectItem value="none">未选择供应商</SelectItem>{values.supplier && !values.supplierId && <SelectItem value="legacy" disabled>历史文本：{values.supplier}</SelectItem>}{suppliers.map(supplier => <SelectItem key={supplier.id} value={String(supplier.id)}>{supplier.code} · {supplier.name}</SelectItem>)}</SelectContent></Select>)}{field("每小时产能（pcs）", <Input type="number" value={values.hourlyCapacity} onChange={event => updateDraft("hourlyCapacity", event.target.value)} onBlur={() => queueSave(values)} disabled={!isAdmin} className={inputClass} placeholder="未录入" />)}{field("OEE（0–1）", <Input type="number" min="0" max="1" step="0.01" value={values.oee} onChange={event => updateDraft("oee", event.target.value)} onBlur={() => queueSave(values)} disabled={!isAdmin} className={inputClass} placeholder="未录入" />)}{field("能耗（kW）", <Input type="number" min="0" value={values.energyConsumption} onChange={event => updateDraft("energyConsumption", event.target.value)} onBlur={() => queueSave(values)} disabled={!isAdmin} className={inputClass} placeholder="未录入" />)}{field("数量（台）", <Input type="number" min="0" value={values.quantity} onChange={event => updateDraft("quantity", event.target.value)} onBlur={() => queueSave(values)} disabled={!isAdmin} className={inputClass} placeholder="未录入" />)}{field("单价（万元）", <Input type="number" min="0" step="0.01" value={values.unitPrice} onChange={event => updateDraft("unitPrice", event.target.value)} onBlur={() => queueSave(values)} disabled={!isAdmin} className={inputClass} placeholder="未录入" />)}{field("折旧年数", <Input type="number" min="0" value={values.depreciationYears} onChange={event => updateDraft("depreciationYears", event.target.value)} onBlur={() => queueSave(values)} disabled={!isAdmin} className={inputClass} placeholder="未录入" />)}{field("损耗系数", <Input type="number" min="0" step="0.01" value={values.lossFactor} onChange={event => updateDraft("lossFactor", event.target.value)} onBlur={() => queueSave(values)} disabled={!isAdmin} className={inputClass} placeholder="未录入" />)}{field("合计金额（万元）", <p className="mt-1 h-8 pt-1 text-sm font-semibold text-[#405342]">{amount === null || Number.isNaN(amount) ? "未录入" : amount.toFixed(2)}</p>)}{field("计入投资", <Button type="button" variant="outline" disabled={!isAdmin} onClick={commitInvestment} className={`mt-1 h-8 w-full justify-start border-[#d9e5d6] text-sm ${values.investmentIncluded ? "bg-emerald-50 text-emerald-800" : "text-[#526652]"}`}>{values.investmentIncluded ? "是" : "否"}</Button>)}</div>{Number(values.oee) < 0.9 && values.oee !== "" && <label className="mt-3 block rounded-xl border border-amber-100 bg-amber-50 p-3 text-xs font-medium text-amber-900">OEE 偏低原因<Textarea value={values.lowOeeReason} onChange={event => updateDraft("lowOeeReason", event.target.value)} onBlur={() => queueSave(values)} disabled={!isAdmin} className="mt-1 min-h-16 border-amber-200 bg-white text-sm text-[#405342]" placeholder="请说明 OEE 偏低原因" /></label>}<label className="mt-3 block rounded-xl border border-[#e4eee0] bg-[#fbfdf9] p-3 text-xs font-medium text-[#7c8c7b]">备注<Textarea value={values.notes} onChange={event => updateDraft("notes", event.target.value)} onBlur={() => queueSave(values)} disabled={!isAdmin} className="mt-1 min-h-20 border-[#d9e5d6] bg-white text-sm text-[#405342]" placeholder="未录入" /></label></div>;
}
...
function BusinessUnitOverview({ businessUnits, equipment, selectedCode, onSelect, language }: { businessUnits: any[]; equipment: any[]; selectedCode: string | null; onSelect: (code: string) => void; language: "zh" | "en" }) {
  ...
  return <section className="mb-5 grid gap-4 md:grid-cols-2 xl:grid-cols-4" aria-label={language === "en" ? "Business unit filters" : "业务单元筛选"}>{businessUnitCards.map(card => {
    const businessUnit = businessUnits.find(item => item.code === card.code);
    const rows = businessUnit ? equipment.filter(item => item.businessUnitId === businessUnit.id) : [];
...
function EquipmentView({ isAdmin }: { isAdmin: boolean }) {
  ...
  const masterData = trpc.equipment.masterData.useQuery();
  const selectedBusinessUnit = useMemo(() => (masterData.data?.businessUnits ?? []).find(item => item.code === selectedBusinessUnitCode), [masterData.data?.businessUnits, selectedBusinessUnitCode]);
  ...
  useEffect(() => {
    ...
    const onRowClick = (event: Event) => {
      ...
      const item = (result.data?.items ?? []).find(entry => entry.code === code);
      if (item) setExpandedEquipmentId(current => current === item.id ? null : item.id);
    };
...
function MasterDataPanel() {
  ...
  const addBu = () => { const code = window.prompt("BU 编码，例如 BU1"); const name = code ? window.prompt("BU 名称") : null; if (code && name) createBusinessUnit.mutate({ code, name }); };
  const addFactory = () => { const code = window.prompt("工厂编码，例如 F01"); const name = code ? window.prompt("工厂名称") : null; const businessUnitCode = name ? window.prompt("所属 BU 编码（可留空，例如 BU1）")?.trim().toUpperCase() : null; const businessUnit = businessUnitCode ? (businessUnits.data ?? []).find(item => item.code.toUpperCase() === businessUnitCode) : undefined; if (businessUnitCode && !businessUnit) { toast.error("未找到对应 BU，请先创建 BU 或留空。 "); return; } if (code && name) createFactory.mutate({ code, name, businessUnitId: businessUnit?.id ?? null }); };
  const addSupplier = () => { const code = window.prompt("供应商编码，例如 SUP-001"); const name = code ? window.prompt("供应商名称") : null; const contactName = name ? window.prompt("联系人（可留空）") ?? undefined : undefined; const phone = name ? window.prompt("联系电话（可留空）") ?? undefined : undefined; if (code && name) createSupplier.mutate({ code, name, contactName: contactName || undefined, phone: phone || undefined }); };
  const editSupplier = (supplier: { id: number; code: string; name: string; contactName: string | null; phone: string | null }) => { const code = window.prompt("供应商编码", supplier.code); const name = code ? window.prompt("供应商名称", supplier.name) : null; const contactName = name ? window.prompt("联系人（可留空）", supplier.contactName ?? "") : null; const phone = name ? window.prompt("联系电话（可留空）", supplier.phone ?? "") : null; if (code && name) updateSupplier.mutate({ id: supplier.id, values: { code, name, contactName: contactName || null, phone: phone || null } }); };
  return <article className="industrial-card overflow-hidden xl:col-span-2"><div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#e5eee2] px-5 py-4"><div><h2 className="font-semibold text-[#26392a]">后台基础数据管理</h2><p className="mt-1 text-xs text-[#829081]">维护设备详情可选的 BU、工厂与供应商信息；创建工厂时可指定其所属 BU。</p></div><div className="flex flex-wrap gap-2"><Button size="sm" variant="outline" onClick={addBu}>新增 BU</Button><Button size="sm" variant="outline" onClick={addFactory}>新增工厂</Button><Button size="sm" className="bg-[#4a7c59] text-white" onClick={addSupplier}>新增供应商</Button></div></div><div className="grid gap-4 p-5 lg:grid-cols-3"><div><p className="mb-2 text-sm font-medium text-[#415843]">BU 列表</p>{businessUnits.data?.length ? <div className="space-y-2">{businessUnits.data.map(item => <div key={item.id} className="rounded-xl bg-[#f4f8f1] px-3 py-2 text-sm"><b>{item.code}</b><span className="ml-2 text-[#687b69]">{item.name}</span></div>)}</div> : <p className="text-sm text-[#829081]">暂无 BU，请新增。</p>}</div><div><p className="mb-2 text-sm font-medium text-[#415843]">工厂列表</p>{factories.data?.length ? <div className="space-y-2">{factories.data.map(item => { const businessUnit = item.businessUnitId == null ? undefined : (businessUnits.data ?? []).find(unit => unit.id === item.businessUnitId); return <div key={item.id} className="rounded-xl bg-[#f4f8f1] px-3 py-2 text-sm"><b>{item.code}</b><span className="ml-2 text-[#687b69]">{item.name}</span><span className="ml-2 text-xs text-[#829081]">{businessUnit ? `· ${businessUnit.code}` : "· 未归属 BU"}</span></div>; })}</div> : <p className="text-sm text-[#829081]">暂无工厂，请新增。</p>}</div><div><p className="mb-2 text-sm font-medium text-[#415843]">供应商列表</p>{suppliers.data?.length ? <div className="space-y-2">{suppliers.data.map(item => <div key={item.id} className="flex items-center justify-between gap-2 rounded-xl bg-[#f4f8f1] px-3 py-2 text-sm"><div className="min-w-0"><b>{item.code}</b><span className="ml-2 text-[#687b69]">{item.name}</span>{item.contactName && <span className="ml-2 text-xs text-[#829081]">· {item.contactName}</span>}</div><div className="flex shrink-0 gap-1"><Button size="sm" variant="ghost" onClick={() => editSupplier(item)} className="h-7 px-2 text-[#56745b]">编辑</Button><Button size="sm" variant="ghost" onClick={() => { if (confirm(`确定删除供应商“${item.name}”吗？设备将保留现有供应商名称。`)) deleteSupplier.mutate({ id: item.id }); }} className="h-7 px-2 text-rose-600">删除</Button></div></div>)}</div> : <p className="text-sm text-[#829081]">暂无供应商，请新增。</p>}</div></div></article>;
}
