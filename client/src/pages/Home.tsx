...
const emptyEquipment: EquipmentForm = {
  ...
  location: "",
  status: "running",
  supplier: "", supplierId: "", businessUnitId: "", factoryId: "", productId: "", assetCategory: "", criticality: "", responsibleOwner: "", commissionedAt: "", warrantyExpiresAt: "", hourlyCapacity: "", oee: "", lowOeeReason: "", energyConsumption: "", quantity: "", unitPrice: "", depreciationYears: "", lossFactor: "", investmentIncluded: false, notes: "",
};
...
const equipmentExportRow = (item: any, businessUnits: any[] = [], factories: any[] = [], suppliers: any[] = []) => ({
  ...
  "每小时产能（pcs）": item.hourlyCapacity, "OEE": item.oee, "OEE偏低原因": item.lowOeeReason, "能耗（kW）": item.energyConsumption, "数量（台）": item.quantity, "单价（万元）": item.unitPrice, "折旧年数": item.depreciationYears, "损耗系数": item.lossFactor, "计入投资": item.investmentIncluded === null ? "" : item.investmentIncluded ? "是" : "否", "备注": item.notes,
});
...
export function InlineEquipmentDetailEditor({ item, businessUnits, factories, suppliers, isAdmin, onSave }: { item: any; businessUnits: any[]; factories: any[]; suppliers: any[]; isAdmin: boolean; onSave: (values: EquipmentForm) => Promise<unknown> }) {
  ...
  const commitInvestment = () => { const nextValues = { ...valuesRef.current, investmentIncluded: !valuesRef.current.investmentIncluded }; valuesRef.current = nextValues; setValues(nextValues); queueSave(nextValues); };
  ...
  return <div className="rounded-2xl border border-[#dbe9d7] bg-white p-4 shadow-sm"><div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-semibold text-[#34513a]">{item.code} · 设备详情</p><p className="mt-1 text-xs text-[#829081]">{isAdmin ? "可直接编辑运营与投资字段；切换选择项或离开输入框后系统自动保存。" : "设备运营与投资字段由管理员维护。"}</p></div><span className={`text-xs font-medium ${saveClassName}`}>{saveState === "saving" && <Loader2 className="mr-1 inline h-3.5 w-3.5 animate-spin" />}{saveStatus}</span></div><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{field("所属 BU", <Select value={values.businessUnitId || "none"} onValueChange={commitBusinessUnit} disabled={!isAdmin}><SelectTrigger className={inputClass}><SelectValue /></SelectTrigger><SelectContent><SelectItem value="none">未选择 BU</SelectItem>{businessUnits.map(unit => <SelectItem key={unit.id} value={String(unit.id)}>{unit.code}</SelectItem>)}</SelectContent></Select>)}{field("工厂", <Select value={values.factoryId || "none"} onValueChange={commitFactory} disabled={!isAdmin}><SelectTrigger className={inputClass}><SelectValue /></SelectTrigger><SelectContent><SelectItem value="none">未选择工厂</SelectItem>{availableFactories.map(factory => <SelectItem key={factory.id} value={String(factory.id)}>{factory.code}</SelectItem>)}</SelectContent></Select>)}{field("供应商", <Select value={values.supplierId || (values.supplier ? "legacy" : "none")} onValueChange={commitSupplier} disabled={!isAdmin}><SelectTrigger className={inputClass}><SelectValue /></SelectTrigger><SelectContent><SelectItem value="none">未选择供应商</SelectItem>{values.supplier && !values.supplierId && <SelectItem value="legacy" disabled>历史文本：{values.supplier}</SelectItem>}{suppliers.map(supplier => <SelectItem key={supplier.id} value={String(supplier.id)}>{supplier.code} · {supplier.name}</SelectItem>)}</SelectContent></Select>)}{field("每小时产能（pcs）", <Input type="number" value={values.hourlyCapacity} onChange={event => updateDraft("hourlyCapacity", event.target.value)} onBlur={() => queueSave(valuesRef.current)} disabled={!isAdmin} className={inputClass} placeholder="未录入" />)}{field("OEE（0–1）", <Input type="number" min="0" max="1" step="0.01" value={values.oee} onChange={event => updateDraft("oee", event.target.value)} onBlur={() => queueSave(valuesRef.current)} disabled={!isAdmin} className={inputClass} placeholder="未录入" />)}{field("能耗（kW）", <Input type="number" min="0" value={values.energyConsumption} onChange={event => updateDraft("energyConsumption", event.target.value)} onBlur={() => queueSave(valuesRef.current)} disabled={!isAdmin} className={inputClass} placeholder="未录入" />)}{field("数量（台）", <Input type="number" min="0" value={values.quantity} onChange={event => updateDraft("quantity", event.target.value)} onBlur={() => queueSave(valuesRef.current)} disabled={!isAdmin} className={inputClass} placeholder="未录入" />)}{field("单价（万元）", <Input type="number" min="0" step="0.01" value={values.unitPrice} onChange={event => updateDraft("unitPrice", event.target.value)} onBlur={() => queueSave(valuesRef.current)} disabled={!isAdmin} className={inputClass} placeholder="未录入" />)}{field("折旧年数", <Input type="number" min="0" value={values.depreciationYears} onChange={event => updateDraft("depreciationYears", event.target.value)} onBlur={() => queueSave(valuesRef.current)} disabled={!isAdmin} className={inputClass} placeholder="未录入" />)}{field("损耗系数", <Input type="number" min="0" step="0.01" value={values.lossFactor} onChange={event => updateDraft("lossFactor", event.target.value)} onBlur={() => queueSave(valuesRef.current)} disabled={!isAdmin} className={inputClass} placeholder="未录入" />)}{field("合计金额（万元）", <p className="mt-1 h-8 pt-1 text-sm font-semibold text-[#405342]">{amount === null || Number.isNaN(amount) ? "未录入" : amount.toFixed(2)}</p>)}{field("计入投资", <Button type="button" variant="outline" disabled={!isAdmin} onClick={commitInvestment} className={`mt-1 h-8 w-full justify-start border-[#d9e5d6] text-sm ${values.investmentIncluded ? "bg-emerald-50 text-emerald-800" : "text-[#526652]"}`}>{values.investmentIncluded ? "是" : "否"}</Button>)}</div>{Number(values.oee) < 0.9 && values.oee !== "" && <label className="mt-3 block rounded-xl border border-amber-100 bg-amber-50 p-3 text-xs font-medium text-amber-900">OEE 偏低原因<Textarea value={values.lowOeeReason} onChange={event => updateDraft("lowOeeReason", event.target.value)} onBlur={() => queueSave(valuesRef.current)} disabled={!isAdmin} className="mt-1 min-h-16 border-amber-200 bg-white text-sm text-[#405342]" placeholder="请说明 OEE 偏低原因" /></label>}<label className="mt-3 block rounded-xl border border-[#e4eee0] bg-[#fbfdf9] p-3 text-xs font-medium text-[#7c8c7b]">备注<Textarea value={values.notes} onChange={event => updateDraft("notes", event.target.value)} onBlur={() => queueSave(valuesRef.current)} disabled={!isAdmin} className="mt-1 min-h-20 border-[#d9e5d6] bg-white text-sm text-[#405342]" placeholder="未录入" /></label></div>;
}
