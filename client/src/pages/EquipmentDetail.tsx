...
import { Activity, ArrowLeft, BarChart3, Building2, CalendarClock, CircleDollarSign, Factory, Gauge, Pencil, ShieldCheck, TrendingUp, Zap } from "lucide-react";
...
function DetailCell({ label, value }: { label: string; value: string | number | null | undefined }) {
  return (
    <div className="rounded-2xl border border-[#e0eadc] bg-[#fbfdf9] p-4">
      <p className="text-xs font-medium tracking-wide text-[#7c8c7b]">{label}</p>
      <p className="mt-2 break-words text-base font-semibold text-[#314a36]">{value === null || value === undefined || value === "" ? empty : value}</p>
    </div>
  );
}
...
function DetailEditor({ item, businessUnits, factories, suppliers, onClose }: { item: any; businessUnits: any[]; factories: any[]; suppliers: any[]; onClose: () => void }) {
  ...
  const textField = (label: string, key: keyof typeof draft, type = "text") => (
    <label className="space-y-1.5"><span className="text-sm font-medium text-[#405342]">{label}</span><Input type={type} value={draft[key] as string | number} onChange={event => setDraft(current => ({ ...current, [key]: event.target.value }))} className="border-[#d9e5d6] bg-white" /></label>
  );
...
function SnapshotTrendChart({ label, value, color, target, noValue, language }: { label: string; value: number | null; color: string; target?: number; noValue: string; language: "zh" | "en" }) {
  const data = value === null ? [] : [{ point: language === "en" ? "Current" : "当前", value }];
  if (!data.length) return <div className="flex h-44 items-center justify-center rounded-2xl border border-dashed border-[#d9e5d6] bg-[#fbfdf9] px-6 text-center text-sm text-[#829081]">{noValue}</div>;
  const axisDomain = target ? [0, 100] : [0, Math.max((value ?? 0) * 1.2, 1)];
  return <div className="h-44"><ResponsiveContainer width="100%" height="100%"><LineChart data={data} margin={{ left: -18, right: 14, top: 18, bottom: 2 }}><CartesianGrid vertical={false} stroke="#e7efe5" /><XAxis dataKey="point" axisLine={false} tickLine={false} tick={{ fontSize: 11, fill: "#829081" }} /><YAxis domain={axisDomain} axisLine={false} tickLine={false} tick={{ fontSize: 11, fill: "#829081" }} /><Tooltip cursor={{ stroke: "#bed8bc", strokeWidth: 1 }} contentStyle={{ borderRadius: 14, border: "1px solid #d9e5d6", boxShadow: "0 8px 20px rgba(43, 74, 52, 0.08)" }} /><Line type="monotone" dataKey="value" name={label} stroke={color} strokeWidth={3} dot={{ r: 6, fill: color, stroke: "#fff", strokeWidth: 3 }} activeDot={{ r: 7 }} />{target ? <ReferenceLine y={target} stroke="#d39b3e" strokeDasharray="4 4" /> : null}</LineChart></ResponsiveContainer></div>;
}

function EquipmentOperationalTrends({ item, language }: { item: any; language: "zh" | "en" }) {
  const isEnglish = language === "en";
  const copy = isEnglish ? { title: "OEE & Energy Trend Analysis", subtitle: "Operational performance snapshot", current: "Current", oee: "OEE", energy: "Energy (kW)", oeeTarget: "OEE target 90%", noOee: "No current OEE value", noEnergy: "No current energy value", note: "This view uses the current equipment record only. Historical curves will expand automatically after time-series readings are available.", healthy: "At or above target", attention: "Below target" } : { title: "OEE 与能耗趋势分析", subtitle: "设备运行表现快照", current: "当前", oee: "OEE", energy: "能耗（kW）", oeeTarget: "OEE 目标 90%", noOee: "暂无当前 OEE 数值", noEnergy: "暂无当前能耗数值", note: "当前仅展示设备台账中的实时快照，接入历史采集读数后将自动延展为时间趋势曲线。", healthy: "达到目标", attention: "低于目标" };
  const oee = item.oee === null || item.oee === undefined ? null : Math.round(Number(item.oee) * 1000) / 10;
  const energy = item.energyConsumption === null || item.energyConsumption === undefined ? null : Number(item.energyConsumption);
  const snapshotTime = new Date(item.updatedAt).toLocaleString(isEnglish ? "en-US" : "zh-CN", { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" });
  return <section className="industrial-card overflow-hidden"><div className="flex flex-col gap-3 border-b border-[#e0eadc] bg-[linear-gradient(135deg,#f7fbf5_0%,#eef7eb_100%)] px-5 py-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-start gap-3"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-[#dcebd8] text-[#3f734d]"><TrendingUp className="h-5 w-5" /></div><div><h2 className="font-semibold text-[#26392a]">{copy.title}</h2><p className="mt-0.5 text-xs text-[#708170]">{copy.subtitle} · {snapshotTime}</p></div></div><Badge variant="outline" className="w-fit border-[#b9d5b8] bg-white/80 text-[#48704e]">{copy.current}</Badge></div><div className="grid gap-5 p-5 lg:grid-cols-2"><article className="rounded-2xl border border-[#e0eadc] bg-white p-4"><div className="mb-3 flex items-start justify-between gap-3"><div><p className="text-sm font-semibold text-[#314a36]">{copy.oee}</p><p className="mt-1 text-xs text-[#829081]">{oee === null ? copy.noOee : `${oee}% · ${oee >= 90 ? copy.healthy : copy.attention}`}</p></div><Gauge className="h-5 w-5 text-[#4a7c59]" /></div><SnapshotTrendChart label={copy.oee} value={oee} color="#4a7c59" target={90} noValue={copy.noOee} language={language} /><p className="mt-2 text-right text-[10px] font-medium text-[#a76d12]">{copy.oeeTarget}</p></article><article className="rounded-2xl border border-[#e0eadc] bg-white p-4"><div className="mb-3 flex items-start justify-between gap-3"><div><p className="text-sm font-semibold text-[#314a36]">{copy.energy}</p><p className="mt-1 text-xs text-[#829081]">{energy === null ? copy.noEnergy : `${energy} kW`}</p></div><Zap className="h-5 w-5 text-[#3999a3]" /></div><SnapshotTrendChart label={copy.energy} value={energy} color="#3999a3" noValue={copy.noEnergy} language={language} /></article></div><p className="border-t border-[#e7efe5] bg-[#fbfdf9] px-5 py-3 text-xs leading-5 text-[#728271]">{copy.note}</p></section>;
}
